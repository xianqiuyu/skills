...
# Amazon SP-API 平台数据（zn-eco）

用 **zn-eco** 渐进式发现 `amazon-sp-api`，再调用代理。不背接口、不驱动浏览器。
...
1. 文件或报告 URL → 下载到当前环境可用的临时目录 → 使用当前环境实际可用的永久文件上传能力 → 只向用户提供永久 `file_url`。禁止提供平台临时 URL 或本地临时路径。
...
```bash
zn-eco http GET /proxy/na/orders/v0/orders \
...
1. **确定店铺**：使用 `browser_id` 作为 `X-Account-Id`；只使用 `amazon-sp-api`。
2. **渐进发现**：path 必须来自上一级发现结果，不得编造。

```bash
zn-eco skills
zn-eco skills get --path "amazon-sp-api"
zn-eco skills get --path "amazon-sp-api <业务模块>"
zn-eco skills get --path "amazon-sp-api <业务模块> operations <操作名>"
```
...
4. 下载到当前环境可用的临时目录，必要时解压或转换，再通过当前环境实际可用的永久文件上传能力上传。
5. 只向用户提供永久文件链接，不提供报告标识、内部 path、本地路径或平台临时 URL。
6. 自首次请求起十分钟仍未完成时停止轮询，告知用户报告仍在生成，可稍后重试。

状态为 `FATAL`、`FAILED`、`CANCELLED` 或等价失败状态时，如实告知，不得凑数。

## 文件类结果

文件必须先下载到当前环境可用的临时目录，再使用当前环境实际存在的永久上传工具或能力上传，并从上传结果中取得永久 `file_url`。
...
- 只使用 `zn-eco skills` 能发现的 `amazon-sp-api` 能力。
